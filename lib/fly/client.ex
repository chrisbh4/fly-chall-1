defmodule Fly.Client do
  ...
  def fetch_nearest_region(config) do
    """
      query {
        nearestRegion {
          name
    ...
    """
    |> perform_query(%{}, config, :fetch_nearest_region)
    |> handle_response()
    |> case do
      {:ok, %{"nearestRegion" => nearest_region}} ->
        Logger.info("nearest_region returned: #{inspect(nearest_region)}")
        # IO.puts(n)
        {:ok, nearest_region}
      ...
      other ->
        Logger.error("Unexpected result from fetch_nearest_region. Response: #{inspect(other)}")

        {:error, "Failed to fetch nearest region"}
    end
  end

  def fetch_regions(config) do
    """
      query {
        platform {
          regions {
    ...
    """
    |> perform_query(%{}, config, :fetch_regions)
    |> handle_response()
    |> case do
      {:ok, %{"platform" => %{"regions" => regions}}} ->
        Logger.info("regions returned: #{inspect(regions)}")
        {:ok, regions}
      ...
      other ->
        Logger.error("Unexpected result from fetch_regions. Response: #{inspect(other)}")
  ...
  def fetch_status(name, config) do
    """
    query ($name: String!, $completed: Boolean!){
      app(name: $name) {
        id
        name
        deployed
        ...
        allocations(showCompleted: $completed) {
          id
          idShort
          version
          latestVersion
    ...
    """
    |> perform_query(%{name: name, completed: true}, config, :fetch_status)
    |> handle_response()
    |> case do
      {:ok, %{"app" => status}} ->
        Logger.info("status returned: #{inspect(status)}")

        status =
          if Kernel.length(status["allocations"]) == 0 do
            Map.put(status, "allocations", nil)
          else
            status
          end
